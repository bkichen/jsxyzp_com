...
function Init() {
	...
	var authkey = "niuniu_web_jxy_" + year + month + day;

	captureObj.NiuniuAuthKey = $.md5(authkey);

	// captureObj.NiuniuAuthKey = "F85084D23817B34FC820B1CB4D5ECAB5";
	// 此处可以设置相关参数
...
function PluginLoadedCallback(success) {
	if (success) {
		$('#info').html('');
		$('#imgshow').hide();
		$('#imgshow').attr('src', "./niuNiu/image/loading.gif?v=1");
		$('#btnReload').hide();
		$('#btnCapture').show();
	}
}
...
function ShowDownLoad() {
	$('#info').html('如果超过5秒无响应，请点此<a target="_blank" href="http://' + window.location.host + '/FS_WEB_CCS/upload/CaptureInstall.exe?" + date.getMinutes() + date.getSeconds()">安装</a>');
}
...
function ReloadPlugin() {
	...
	if (captureObj.pluginValid()) {
		...
		var browserInfo = "查看控件是否被浏览器阻止，或通过浏览器设置中的加载项查看Capture是否加载并正常运行";
		$('#info').html('截图控件未能识别到，请按如下步骤检查:<br/>1. 确定您已经下载控件安装包并正常安装 <br/>2. ' + browserInfo + '<br/>3. 刷新页面或重新启动浏览器试下');
	}
}
...
function StartCapture() {
...

	{
		...
		$('#info').html('正在截图中, 如果超过5秒无响应，请点此<a target="_blank" href="http://' + window.location.host + '/FS_WEB_CCS/upload/CaptureInstall.exe?" + date.getMinutes() + date.getSeconds()">安装</a>');
	}
}
...
function UploadData() {
	...
	$.ajax({
		...
		url : "api/agent/uploadPic2Server",
		dataType : "json",
